...
\item \textbf{EVM*:} Runs code on the blockchain
...
\textbf{The Narrative}
\begin{itemize}\compactlist
\item Platform for dApps*
\item ``DeFi'' -- finance without banks
\item NFTs, DAOs, and more
\item Base layer for Web3
\end{itemize}

{\scriptsize *EVM = Ethereum Virtual Machine (the ``computer'' running smart contracts). Composable = can be combined like LEGO bricks. dApps = decentralized applications.}
\end{column}
\begin{column}{0.42\textwidth}
\begin{center}
...
\textit{You won't need to program -- this is just to understand what happens behind the scenes.}
...
\item Developer writes contract in Solidity*
\item Code compiles to bytecode**
...
{\scriptsize *Solidity = programming language for Ethereum smart contracts. **Bytecode = machine-readable instructions. ***Sandboxed = runs in an isolated environment for safety.}
...
\vspace{2mm}
{\scriptsize *Yield farming = earning rewards by lending or providing liquidity to DeFi protocols. **Derivatives = financial contracts whose value depends on an underlying asset. ***Tokenized assets = real-world assets (property, stocks) represented as blockchain tokens.}
...
\begin{columns}[T]
\begin{column}{0.48\textwidth}
\textbf{Bitcoin:} Rare, carefully tested upgrades
\begin{itemize}\compactlist
\item Only a handful of major changes since 2009
\item Years of community debate before each one
\item Philosophy: stability and predictability above all
\end{itemize}
\end{column}
\begin{column}{0.48\textwidth}
\textbf{Ethereum:} Frequent upgrades adding new features
\begin{itemize}\compactlist
\item New fee system (2021)
\item Switched from PoW to PoS (2022)
\item Regular hard forks with new capabilities
\item Philosophy: improve and adapt continuously
\end{itemize}
\end{column}
\end{columns}
\end{frame}
