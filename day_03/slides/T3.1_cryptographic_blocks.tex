...
{\scriptsize SHA-256 = Secure Hash Algorithm producing a 256-bit output. Hexadecimal = a number system using 0--9 and a--f, common in computing.}
\end{frame}
...
\textit{Don't memorize all five -- the key insight is that hash functions are \textbf{one-way} and produce \textbf{unique} outputs.}
...
\begin{columns}[T]
\begin{column}{0.55\textwidth}
\textbf{Three Key Uses in Blockchain}
\begin{itemize}\compactlist
\item \textbf{Data integrity:} Verify nothing changed -- if the hash matches, the data is untouched
\item \textbf{Block linking:} Each block contains the hash of the previous block, creating a tamper-evident chain
\item \textbf{Efficient verification:} Hash-based structures (covered next) let you check any single transaction with only a handful of checks, even among thousands
\end{itemize}
\end{column}
\begin{column}{0.42\textwidth}
\begin{center}
...
\footnotesize\textbf{Hash-Based Verification}\\
...
\textit{You don't need to understand the math. The key point is: blockchain uses a specific type of digital signature that is compact and secure.}

\vspace{3mm}
\textbf{Why do blockchains use elliptic curve signatures?}

\vspace{2mm}
There are different mathematical approaches to creating digital signatures. Blockchains chose \textbf{Elliptic Curve Digital Signature Algorithm (ECDSA)} because it provides strong security with much smaller keys:

\vspace{2mm}
\begin{center}
\begin{tabular}{lcc}
\toprule
\textbf{Property} & \textbf{Older method (RSA)} & \textbf{Blockchain method (ECDSA)} \\
\midrule
...
\bottomnote{In NB05, we use RSA for simplicity, but real blockchains use ECDSA for efficiency}
\end{frame}
...
\vspace{3mm}
\textbf{A blockchain transaction contains four essential pieces:}
...
\textbf{Digital signature} & Proves the sender authorized this transfer \\
\hline
\end{tabular}
\end{center}
...
\bottomnote{Technical details like gas fees and transaction ordering are covered in Topic 3.3.}
\end{frame}
...
\hrule
\vspace{3mm}
\textbf{Question 3:} Why is it important that hash functions are one-way?

\pause
\textbf{Answer:} Because someone who sees a hash cannot reverse-engineer the original data. This protects transaction details, passwords, and other sensitive information on the blockchain.
\end{frame}
