...
% (aibox style inherited from template_beamer.tex line 148)
...
\item Permissionless, pseudonymous (identities hidden behind addresses, but transactions visible)
...
\vspace{0.3cm}
{\footnotesize \textbf{Quick refresher from earlier days} (don't worry if you don't remember everything---we will revisit key ideas as we go)\textbf{:} Day~1 introduced \emph{what money is} and the FinTech vs.\ DeFi distinction shown above. Day~2 covered \emph{digital payments, APIs, and platform economics}. Day~3 explained \emph{blockchain cryptography, wallets, and Bitcoin vs.\ Ethereum}. Day~4 explored \emph{smart contracts, DeFi lending/trading, stablecoins, and tokenization}. Day~5 examined \emph{risks, regulation, DAOs, and privacy}.}
...
\begin{itemize}
\item \textbf{2017--2020}: Clear separation
  \begin{itemize}
  \item ICO boom (2017)
  \item Crypto winter (2018)
  \item DeFi Summer begins (2020)
  \end{itemize}
\item \textbf{2021--2024}: Boundaries blur
  \begin{itemize}
  \item PayPal adds crypto buying (2021)
  \item Visa settles in USDC (2021)
  \item Aave Arc launches permissioned DeFi (2022)
  \item BlackRock files Bitcoin ETF (2023)
  \item Spot Bitcoin ETFs approved in the US (Jan 2024)
  \end{itemize}
\item \textbf{2025+}: Convergence accelerates---hybrid models dominate new product launches
\end{itemize}
\end{frame}
...
\node[font=\bfseries] at (-3.8,1.5) {FinTech (blue)};

% Right circle - DeFi
\draw[thick, dfteal, fill=dfteal!20] (2.5,0) circle (2.2cm);
\node[font=\bfseries] at (3.8,1.5) {DeFi (teal)};
...
\node[align=center, font=\small\bfseries, text=dfred] at (0,0) {Convergence\\Zone\\(orange)};
...
\item Fiat (government-issued money like USD or EUR) on-ramp/off-ramp (converting between traditional money and crypto)
...
\textbf{Why banks care:} Operational efficiency, reduced counterparty risk (the danger that the other side of a deal defaults)
\end{frame}
...
Global assets under management exceeded \$120\,trillion in 2024 (BCG Global Asset Management Report). A growing share of these institutional investors want blockchain exposure but require:
\end{block}

\begin{itemize}
\item \textbf{Regulatory clarity}: Clear legal status of assets
\item \textbf{Custody solutions}: Qualified custodians (firms licensed to hold assets on behalf of clients) with insurance
\item \textbf{Counterparty verification}: KYC/AML (Know Your Customer / Anti-Money Laundering---identity checks required by law) compliance
\item \textbf{Risk management}: Understood, quantifiable risks
\item \textbf{Familiar structures}: Funds, ETFs (Exchange-Traded Funds---investment products traded on stock exchanges like shares), and other regulated vehicles
\end{itemize}
...
\item Whitelisted wallet addresses (pre-approved cryptocurrency wallet addresses that have passed identity verification)
...
\begin{block}{Definition}
DeFi protocols or pools designed for institutions with KYC/AML (Know Your Customer / Anti-Money Laundering regulations), permissioned (requiring approval to join) access, and regulatory compliance.
\end{block}
...
\textbf{Key examples:}
\begin{itemize}
\item \textbf{Aave Arc}: Permissioned Aave for institutions (launched 2022; saw limited adoption---institutional DeFi access has since shifted to platforms like Fireblocks and Copper)
...
\begin{frame}{Aave Arc: What It Means for Beginners}
\begin{exampleblock}{Translation for Beginners}
Think of Aave Arc like a \textbf{members-only club version} of an open marketplace. The marketplace (Aave) is the same, but Arc adds a bouncer at the door who checks your ID. Only verified members can enter and trade. The goods and prices inside are identical---only the entry rules differ.
\end{exampleblock}

\vspace{0.3cm}
\textbf{Note:} Aave Arc launched in 2022 but saw limited uptake. The institutional DeFi market has since moved toward custody-first platforms such as Fireblocks and Copper, though the concept of permissioned DeFi pools remains influential.

\begin{alertblock}{Key Takeaway}
The \emph{concept} of permissioned DeFi pools---where identity-verified users access decentralized protocols---remains influential even as specific products come and go.
\end{alertblock}
\end{frame}
...
{\scriptsize *FDIC = Federal Deposit Insurance Corporation, the US government agency that insures bank deposits up to \$250{,}000. Most countries have equivalent schemes (e.g., FSCS in the UK, Einlagensicherung in Germany/EU).}
...
Deposit insurance & Yes (in jurisdictions with schemes, e.g.\ FDIC in the US, FSCS in the UK) & No \\
...
\textbf{Examples:}
\begin{itemize}
\item \textbf{MakerDAO} (rebranded to \textbf{Sky} in 2024, with DAI transitioning to USDS): DAI/USDS backed by RWA vaults
...
\vspace{0.5cm}
\begin{exampleblock}{MakerDAO's RWA Strategy (now Sky)}
MakerDAO (rebranded to Sky in 2024) allocated \$2B+ to real-world assets (US Treasuries, corporate bonds) as of mid-2024. A ``DeFi'' protocol holding TradFi assets---convergence in action. The rebrand also transitioned DAI to a new token called USDS.
\end{exampleblock}
\end{frame}

\begin{frame}{Deep Dive: MakerDAO (now Sky) -- Real-World Assets}
\begin{center}
\begin{tikzpicture}[scale=0.75]
% DAI
\node[blockchain, minimum width=2.5cm] (dai) at (0,2) {DAI / USDS};
...
{\scriptsize \textbf{Key terms:} PSM = Peg Stability Module (a mechanism that lets users swap stablecoins 1:1 to keep DAI/USDS close to \$1). RWA = Real-World Assets (traditional financial assets like bonds, loans, or real estate tokenized on a blockchain).}
\end{frame}
...
\textit{There is a spectrum between full self-custody (you hold all the keys) and full bank custody (the bank holds everything for you). Most real-world solutions now sit somewhere in between.}
...
\end{itemize}
{\footnotesize \textit{Analogy: Keeping cash in a home safe---only you have access, but if you lose the combination, no one can help.}}
\end{column}
\begin{column}{0.48\textwidth}
...
\end{itemize}
{\footnotesize \textit{Analogy: A bank safe-deposit box---the bank secures it for you, but you must trust the bank to stay solvent.}}
\end{column}
\end{columns}
...
\begin{itemize}
\item \textbf{Multi-sig wallets}: 2-of-3 keys (user, institution, backup). \textit{Like a safe that needs two of three keyholders to open.}
\item \textbf{Social recovery}: Designated guardians can restore access. \textit{Like naming trusted friends who can vouch for you if you lose your key.}
\item \textbf{Tiered access}: Small amounts self-custody, large amounts require verification. \textit{Like using a debit card for small purchases but needing extra approval for big ones.}
\item \textbf{MPC (Multi-Party Computation---splitting a secret key among multiple parties)}: Key shares distributed, no single point of failure. \textit{Like splitting a treasure map into pieces held by different people.}
\end{itemize}
\end{block}
\end{frame}
...
\begin{block}{Definition}
Building systems that use \textbf{both} traditional technology and blockchain, picking the best tool for each job---rather than forcing everything onto one platform.
\end{block}

\textbf{Examples:}
\begin{itemize}
\item \textbf{Hybrid databases}: Use a normal database for everyday records, but record final settlements (confirmed transactions) on a blockchain for tamper-proof proof
\item \textbf{Layer 2 solutions}: A fast centralized processor handles transactions quickly, then a decentralized blockchain double-checks the results (as covered in Day~3)
\item \textbf{Private + Public chains}: A company runs its own private blockchain internally, then periodically anchors a summary to a public blockchain for transparency
\item \textbf{API gateways}: A traditional web interface (API) that lets apps connect to decentralized protocols without users needing to understand blockchain directly
\end{itemize}

\vspace{0.3cm}
\begin{alertblock}{Design Principle}
Not everything needs to be on a blockchain. The best systems combine technologies based on what each does best.
\end{alertblock}
\end{frame}

\begin{frame}{Cross-Chain Interoperability: The Connector}

{\footnotesize \textbf{Key terms:} \textit{Interoperability} = different blockchains being able to communicate and transfer assets, like being able to send email between Gmail and Outlook. \textit{Cross-chain} means moving assets or data between different blockchains (e.g., Ethereum to Solana). A \textit{bridge} is software that locks an asset on one chain and issues an equivalent on another.}
...
\item Breaks down liquidity silos (pools of money trapped on a single platform)
...
\textbf{Solutions:}
\begin{itemize}
\item \textbf{Bridges}: Cross-chain asset transfers (but with security challenges---bridge hacks have caused billions in losses)
\item \textbf{Chain abstraction}: Hide blockchain complexity from users so they do not need to know which chain they are on
\item \textbf{Standardized protocols}: Common messaging formats so different chains can ``speak the same language''
\item \textbf{SWIFT integration}: Traditional banking networks connecting to blockchain
\end{itemize}

\vspace{0.2cm}
\begin{exampleblock}{SWIFT + Chainlink}
SWIFT (the messaging network used by 11{,}000+ banks worldwide for cross-border transfers) partnered with Chainlink to enable blockchain interoperability---a direct FinTech-DeFi bridge.
\end{exampleblock}
\end{frame}
...
In March 2024, BlackRock (world's largest asset manager, \$10T+ AUM) launched BUIDL---a tokenized fund for US Treasury exposure on Ethereum. Within months BUIDL attracted over \$500M in assets, making it the largest tokenized Treasury fund.
\end{block}
...
\vspace{0.2cm}
{\footnotesize \textbf{Step-by-step hints:} (1)~Pick one innovation above. (2)~Score each of the five factors as ``high'' or ``low.'' (3)~Count your ``high'' scores---more highs = higher convergence likelihood. (4)~Compare with a classmate's choice and discuss where you disagree.}
\end{frame}
...
\node[blockchain, minimum width=2cm, font=\footnotesize] (maker) at (5,-2) {Maker/Sky};
...
\vspace{1.5cm}
\textcolor{dfgray}{Digital Finance | Joerg Osterrieder | 2026}
\end{center}
\end{frame}
