% =====================================================================
% Topic 6.3: Building Your Digital Finance Worldview
% Course: Digital Finance (BSc) -- Day 6, Topic 3
% CANONICAL SOURCE for the Innovation Scorecard (6 Questions)
% =====================================================================
...
\item Day 2: Digital payments and FinTech platforms
\item Day 3: Blockchain fundamentals
\item Cryptographic foundations
\item Platform economics
\item APIs and data-driven finance
\end{itemize}
\end{column}
\begin{column}{0.48\textwidth}
\textbf{Days 4-6 Applications:}
\begin{itemize}
\item Day 4: Smart contracts and DeFi
\item DeFi primitives and stablecoins
\item Tokenization and CBDCs
\item Day 5: Risks and regulation
\item Privacy and inclusion
\item Day 6: AI and convergence
\end{itemize}
\end{column}
\end{columns}
...
\item What are the dependencies? (Oracles (services that feed real-world data to smart contracts), other protocols, infrastructure)
\end{itemize}
\end{block}
...
\begin{itemize}
\item Recall from Topic 4.2: AMMs (Automated Market Makers) use the constant product formula $x \times y = k$---the pool must always maintain the same total value
...
Banking product & OCC/Fed/FDIC\footnotemark{} oversight \\
\bottomrule
\end{tabular}
\end{table}

\footnotetext{FDIC = Federal Deposit Insurance Corporation (US). Similar schemes: FSCS (UK), Einlagensicherung (EU).}

\vspace{0.2cm}
\footnotesize{\textit{Note: These categories are illustrative and US-focused. Internationally, the EU uses the MiCA framework, the UK has its FCA regime, and Asian markets follow varying approaches (see Topic 5.2). Always check your own jurisdiction.}}
\end{frame}
...
\toprule
\textbf{Topics} & \textbf{Dimension} & \textbf{What It Measures} \\
\midrule
1.1--1.4 (Foundations) & Trust Architecture & How is trust established and maintained? \\
2.1--2.4 (FinTech) & Platform Dynamics & Network effects, ecosystem, switching costs \\
3.1--3.4 (Blockchain) & Technical Soundness & Security, scalability, reliability \\
4.1--4.4 (DeFi) & Programmability & Smart contracts, composability, automation \\
5.1--5.4 (Risk \& Reg.) & Risk Profile & Technical, economic, regulatory risks \\
6.1--6.4 (Future) & Future Potential & Growth trajectory, convergence potential \\
\bottomrule
\end{tabular}
\end{table}
...
% ---------------------------------------------------------------------
\begin{frame}{Dimension 1: Trust Architecture (Topics 1.1--1.4)}
...
% ---------------------------------------------------------------------
\begin{frame}{Dimension 2: Platform Dynamics (Topics 2.1--2.4)}
...
\vspace{0.3cm}
\begin{exampleblock}{Platform Comparison (illustrative scores for class discussion)}
\begin{itemize}
\item \textbf{Stripe}: Strong two-sided network effects, excellent developer ecosystem (5/5)
\item \textbf{Uniswap}: Liquidity network effects, low switching costs (4/5)
\item \textbf{Digital Euro}: Not yet launched, mandatory adoption likely (3/5)
\end{itemize}
\end{exampleblock}
\end{frame}
...
% ---------------------------------------------------------------------
\begin{frame}{Dimension 3: Technical Soundness (Topics 3.1--3.4)}
...
% ---------------------------------------------------------------------
\begin{frame}{Dimension 4: Programmability (Topics 4.1--4.4)}
\textbf{Criteria for Programmability Assessment:}

\begin{enumerate}
\item \textbf{Smart Contract Support}: Native or none?
\item \textbf{Composability}: Can combine with other protocols? (Composability means combining DeFi building blocks like Lego pieces, introduced in Day 4)
...
Composability & API-based & ``Money Legos'' (composable DeFi protocols) & Restricted \\
Score & 4/5 & 5/5 & 3/5 \\
\bottomrule
\end{tabular}

\footnotesize{\textit{Note: Scores are illustrative examples for classroom discussion, not definitive ratings.}}
\end{table}
...
% ---------------------------------------------------------------------
\begin{frame}{Dimension 5: Risk Profile (Topics 5.1--5.4)}
...
\textbf{Impermanent loss} = the potential loss liquidity providers face when token prices change compared to simply holding the tokens. When you deposit tokens into an AMM pool and the price moves, the pool rebalances automatically, leaving you with less value than if you had just kept the tokens in your wallet. This loss is called ``impermanent'' because it reverses if prices return to their original ratio---but it becomes permanent when you withdraw your liquidity.
\end{alertblock}
...
% ---------------------------------------------------------------------
\begin{frame}{Dimension 6: Future Potential (Topics 6.1--6.4)}
...
\item \textbf{ESG} (Environmental, Social, and Governance criteria for responsible investing): Integration with tokenized assets
\end{itemize}
\end{exampleblock}
\end{frame}
...
\item Tokenized treasuries (government bonds represented as digital tokens on blockchain)
...
% Points
\node[circle, fill=dfblue, minimum size=10pt, label=right:{\footnotesize Digital Euro (3, 5)}] at (2.4,4) {};
\node[circle, fill=dforange, minimum size=10pt, label=right:{\footnotesize Stripe (4, 4)}] at (3.04,3.2) {};
\node[circle, fill=dfteal, minimum size=10pt, label=right:{\footnotesize Uniswap (5, 2)}] at (4,1.6) {};
...
% Four lenses
\node[blockchain, minimum width=3cm, font=\footnotesize, align=center] (tech) at (-5,2.5) {Technical Lens\\(Topics 2--3)};
\node[blockchain, minimum width=3cm, font=\footnotesize, align=center] (econ) at (5,2.5) {Economic Lens\\(Topics 1, 4)};
\node[blockchain, minimum width=3cm, font=\footnotesize, align=center] (reg) at (-5,-2.5) {Regulatory Lens\\(Topic 5)};
\node[blockchain, minimum width=3cm, font=\footnotesize, align=center] (social) at (5,-2.5) {Social Lens\\(All Topics)};
...
\begin{enumerate}
\item PROBLEM: What real problem does this solve, and for whom?
\item MECHANISM: How does it actually work (technically and economically)?
\item TRADEOFFS: What are the key tradeoffs and design choices?
\item RISKS: What could go wrong (technical, economic, regulatory)?
\item REGULATORY STATUS: Where does it fit in the regulatory landscape?
\item WHO BENEFITS: Who captures value, and who bears costs?
\end{enumerate}
\end{block}
...
\item[Composability] Ability of protocols to combine like ``money legos'' (a nickname for composable DeFi protocols)---building complex products from simple primitives
...
\vspace{0.5cm}
\textit{Think about your answer before moving to the next slide.}
\end{frame}

% ---------------------------------------------------------------------
% Slide 36b: Self-Assessment Answer 1
% ---------------------------------------------------------------------
\begin{frame}{Self-Assessment Answer 1}
...
\item Topics 1.1--6.4 slide decks (reference for each dimension)
\end{itemize}
