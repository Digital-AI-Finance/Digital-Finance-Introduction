...
\item \textbf{Understand} how robo-advisors make investment decisions (the logic behind automated portfolio balancing)
...
\begin{itemize}
\item Money and financial systems (Day 1)
\item Digital payments and APIs (Day 2)
\item Blockchain and consensus (Day 3)
\item Smart contracts and DeFi (Day 4)
\item Risks and regulation (Day 5)
\end{itemize}
\end{column}
\begin{column}{0.48\textwidth}
\textbf{Course Context:}
\begin{itemize}
\item Day 1: Foundations -- Money, FinTech vs.\ DeFi
\item Day 2: Payments, APIs, Data-Driven Finance
\item Day 3: Cryptography, Blockchain, Wallets
\item Day 4: Smart Contracts, DeFi, Stablecoins
\item Day 5: Risks, Regulation, DAOs, Privacy
...
\begin{itemize}
\item \$1.4T+ in robo-advisor AUM globally (2024, Statista)
\item 75\%+ of US equity trading volume is algorithmic (SEC estimates)
...
\textbf{Scale:}
\begin{itemize}
\item \$1.4T+ AUM globally (2024, Statista)
...
% =====================================================================
\begin{frame}{Robo-Advisory: The Intuition Behind It}
\begin{block}{Modern Portfolio Theory in Plain English}
\textbf{Core idea:} Don't put all your eggs in one basket. By mixing investments that don't move together, you can get better returns for the same level of risk.
\end{block}

\vspace{0.3cm}
\textbf{What the robo-advisor's algorithm does:}
\begin{enumerate}
\item \textbf{Estimates} how much each investment might return and how risky it is
\item \textbf{Measures} how investments move relative to each other (when one goes up, does the other go down?)
\item \textbf{Finds} the best mix given how much risk you are comfortable with
\item \textbf{Adjusts} the mix over time as markets change
\end{enumerate}

\vspace{0.3cm}
\textbf{How Your Risk Tolerance Maps to Investments:}
\begin{table}[h]
\centering
\small
\begin{tabular}{lc}
\toprule
\textbf{Risk Profile} & \textbf{Typical Stock Allocation} \\
\midrule
...
\bottomnote{For the mathematical formulas behind this, see the Appendix. Modern robo-advisors also add constraints: ESG filters, tax efficiency, position limits.}
\end{frame}

% =====================================================================
%                    SLIDE 8: PORTFOLIO RETURN AND VARIANCE
% =====================================================================
\begin{frame}{Portfolio Concepts: Return, Risk, and the Sharpe Ratio}
\textit{These concepts power robo-advisors behind the scenes:}
\vspace{0.2cm}
\begin{columns}[T]
\begin{column}{0.48\textwidth}
...
\centering
\textit{Sharpe Ratio} = \\[0.2cm]
\footnotesize $\dfrac{\text{Portfolio return} - \text{Risk-free return}}{\text{Portfolio risk (volatility)}}$
\end{block}

\begin{itemize}
\item \textbf{Numerator:} How much extra return you earn above a ``safe'' investment (like a government bond)
\item \textbf{Denominator:} How bumpy the ride is
\item \textbf{Higher is better:} More reward per unit of risk
\end{itemize}

{\scriptsize *Sharpe ratio = a measure of return per unit of risk. Higher is better---it tells you how much extra reward you earn for each unit of ``bumpiness'' in your portfolio.}
\end{column}
\end{columns}

\bottomnote{For the full mathematical formulas, see the Appendix.}
\end{frame}
...
% Axes
\draw[->] (0,0) -- (6,0) node[right, font=\footnotesize, text width=2.5cm, align=left] {Risk (how much prices swing)};
\draw[->] (0,0) -- (0,5) node[above, font=\footnotesize] {Expected Return (average gain)};
...
{\scriptsize *Capital gains tax = tax on the profit you make when you sell an investment for more than you paid.\\
...
\vspace{0.2cm}
\begin{alertblock}{International Note}
Tax-loss harvesting is primarily a US concept. International equivalents exist in some countries (e.g., capital gains offsetting in the UK, \textit{Verlustverrechnung} in Germany) but rules vary significantly. The Wash Sale Rule described above is US-specific (IRS).
\end{alertblock}
\end{frame}
...
\textit{AI helps catch fraud that humans would miss---by learning patterns from millions of past transactions and spotting subtle anomalies in real time.}
...
\begin{itemize}
\item \textbf{Supervised}: Random forests, gradient boosting (an AI technique that combines many simple predictions into one strong one), neural networks on labeled fraud data
\item \textbf{Unsupervised}: Anomaly detection using isolation forests (an algorithm that detects unusual patterns by isolating outliers) and autoencoders (AI models that learn to compress and reconstruct data, useful for spotting anomalies)
...
\begin{columns}[T]
\begin{column}{0.48\textwidth}
\textbf{Traditional FICO (a widely used US credit score):}
\begin{itemize}
...
\begin{itemize}
\item \textbf{ECOA} (Equal Credit Opportunity Act, US): Cannot discriminate on race, religion, national origin, sex
\item \textbf{FCRA} (Fair Credit Reporting Act, US): Right to know reasons for adverse actions
\item \textbf{GDPR} (EU): Right to explanation for automated decisions
\end{itemize}
\footnotesize\textit{Note: ECOA and FCRA are US-specific; other jurisdictions have equivalent frameworks.}
\end{alertblock}
\end{frame}
...
\textbf{Evolution:}
\begin{itemize}
\item \textbf{1980s--90s}: Statistical arbitrage (using math to find temporary price differences)
\item \textbf{2000s}: High-frequency trading emerges
...
\item Alpha (returns above the market average) decays quickly once discovered
...
\vspace{0.5cm}
{\scriptsize *Bid-ask spread = the difference between the highest price a buyer will pay and the lowest a seller will accept.\\
...
\begin{itemize}
\item MEV (Maximal Extractable Value) bots -- profit that blockchain validators can extract by reordering transactions, like a postal worker who reads your letters and acts on the information before delivering them
...
Complex ML models (neural networks, gradient boosting) often cannot explain \emph{why} they make specific predictions. This creates regulatory and ethical challenges in finance.
\end{block}
...
\item SHAP/LIME (tools that explain why an AI made a specific decision)
...
\textit{AI in finance creates new types of systemic risk---when many firms rely on similar AI models, their correlated behavior can amplify market shocks.}
...
\bottomnote{Notebook: NB13\_Robo\_Advisor.ipynb}
\end{frame}
...
\begin{enumerate}
\item Calculate expected returns for each asset
\item Estimate how assets move together
\item Find the best portfolio mix
\item Generate the efficient frontier chart
\item Map risk scores to allocations
\item Backtest (test on historical data)
\end{enumerate}
...
\begin{itemize}
\item FinTech product managers
\item Compliance analysts who understand AI
\item Business strategists evaluating AI claims
\item Risk officers assessing algorithmic systems
...
\item[Sharpe Ratio] A scorecard measuring how much extra return you earn per unit of risk -- higher is better
...
\begin{description}
\item[Wash Sale Rule] US IRS rule prohibiting repurchase of substantially identical securities within 30 days of selling at a loss. Other countries have similar concepts (e.g., ``bed and breakfasting'' rules in the UK, \textit{Verlustverrechnung} rules in Germany)
...
\item[SHAP/LIME] Tools that explain \emph{why} an AI made a specific decision -- like showing which factors most influenced a loan approval or denial
\end{description}
\end{frame}
...
\begin{enumerate}[A.]
\item It guarantees maximum returns with zero risk
\item It identifies portfolios that offer the best return for each level of risk
\item It eliminates the need for diversification
\item It predicts future stock prices accurately
\end{enumerate}
\end{block}
...
\item Question 3: \textbf{B} -- The efficient frontier shows the optimal risk-return tradeoff; portfolios below it are suboptimal
\end{itemize}
\end{exampleblock}
\end{frame}
...
\item QuantConnect (algo trading platform, free tier available)
\end{itemize}
\end{column}
\end{columns}
\end{frame}
...
\textcolor{dfgray}{Joerg Osterrieder | Digital Finance | 2026}
\end{center}
\vfill
\end{frame}

% =====================================================================
%              APPENDIX: OPTIONAL MATHEMATICAL DETAILS
% =====================================================================
\appendix

\begin{frame}{Optional Deep Dive: Mathematical Details (1/2)}
\textit{This appendix contains the formal mathematics behind the robo-advisory concepts discussed earlier. This material is optional and not required for the course.}

\vspace{0.3cm}
\textbf{Mean-Variance Optimization (Markowitz, 1952):}
\begin{equation}
\min_w \frac{1}{2} w^T \Sigma w - \lambda \mu^T w
\end{equation}
\begin{itemize}
\item $w$ = portfolio weights (how much of each investment to hold)
\item $\Sigma$ = covariance matrix of returns (measures how investments move relative to each other)
\item $\mu$ = expected returns vector (estimated future gains for each investment)
\item $\lambda$ = risk aversion parameter (higher = more cautious investor)
\end{itemize}

\vspace{0.3cm}
\textbf{In words:} Find the portfolio weights $w$ that minimize risk (first term) while maximizing expected return (second term), with $\lambda$ controlling the trade-off.
\end{frame}

\begin{frame}{Optional Deep Dive: Mathematical Details (2/2)}
\textbf{Portfolio Expected Return:}
\begin{equation}
r_p = w^T \mu = \sum_{i=1}^{n} w_i \mu_i
\end{equation}
\textit{The portfolio return is the weighted average of individual asset returns.}
...
\end{equation}
\textit{Portfolio risk depends on both individual asset risks and how assets move together (correlations). When correlations are below 1, diversification reduces overall risk.}
...
\end{equation}
\textit{Measures excess return (above the risk-free rate $r_f$, e.g.\ government bonds) per unit of risk. Higher Sharpe ratios indicate better risk-adjusted performance.}
\end{frame}
