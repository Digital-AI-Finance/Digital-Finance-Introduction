...
\begin{itemize}
\item Blockchain fundamentals (Day 2-3)
\item Smart contracts and DeFi (Day 3-4)
...
\draw[dfblue] (0,3) -- (4,3);
\node at (2,2.5) {\footnotesize Day 2-3: Technical};
\draw[dfblue] (0,2) -- (4,2);
\node at (2,1.5) {\footnotesize Day 4-5: Applied};
\draw[dfblue] (0,1) -- (4,1);
...
\vspace{0.2cm}
\textbf{Stablecoin market cap:} \$150B+ (2024)
\end{column}
\end{columns}
...
\textbf{Challenges:}
\begin{itemize}
\item Key management for normies
...
\begin{itemize}
\item ECDSA (used by Bitcoin, Ethereum)
\item RSA encryption
...
\vspace{5mm}
\textbf{Post-Quantum Cryptography (NIST Standards):}
\begin{itemize}
\item \textbf{CRYSTALS-Kyber}: Key encapsulation mechanism
\item \textbf{CRYSTALS-Dilithium}: Digital signatures
\item \textbf{SPHINCS+}: Hash-based signatures (conservative choice)
\end{itemize}
...
Makes blockchain accounts programmable, dramatically improving user experience.
\end{block}
...
Using token incentives to coordinate real-world infrastructure deployment.
\end{block}
...
% Labels
\node[below, text width=3cm, align=center] at (-4,-1.2) {\footnotesize China, some emerging markets};
\node[below, text width=3cm, align=center] at (0,-1.2) {\footnotesize EU (MiCA), UK, Singapore};
\node[below, text width=3cm, align=center] at (4,-1.2) {\footnotesize UAE, some US states};
\end{tikzpicture}
\end{center}
...
\begin{block}{Five Questions for Any Digital Finance Innovation}
\end{block}
...
\item \textbf{MECHANISM}: How does it actually work? What's the trust model?\\
\footnotesize \textcolor{dfgray}{Red flag: ``It's decentralized'' without specifics}

\item \textbf{INCENTIVES}: Who profits and how? Are incentives aligned?\\
\footnotesize \textcolor{dfgray}{Red flag: Unclear value capture or ponzinomic structures}

\item \textbf{RISKS}: What can go wrong? What are the trade-offs?\\
\footnotesize \textcolor{dfgray}{Red flag: Claims of ``no risk'' or ``guaranteed returns''}

\item \textbf{REGULATORY}: Where does this fit legally?\\
...
\midrule
1 & Landscape & FinTech vs. DeFi framing; market structure; key players \\
2 & Digital Payments & Payment systems; API economy; data-driven finance \\
3 & Blockchain & Consensus, security, scalability trilemma; cryptographic foundations \\
4 & Smart Contracts & Programmability, composability, DeFi primitives (AMMs, lending) \\
5 & Regulation & Global approaches; compliance frameworks; legal status \\
6 & Future & Convergence; AI applications and risks; evaluation framework \\
\bottomrule
\end{tabular}
\end{table}
\end{frame}
...
\item[Tokenized Deposits] Bank deposits represented on blockchain, maintaining FDIC insurance and bank liability status.
