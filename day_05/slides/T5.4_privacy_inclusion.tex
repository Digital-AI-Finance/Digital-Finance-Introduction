...
\item \textbf{Analyze} CBDC (Central Bank Digital Currency) privacy implications and programmable money
...
% ============================================================================
% SLIDE 2b: Transition from T5.3 (H60)
% ============================================================================
\begin{frame}{Why This Topic Now?}
\begin{block}{Connection to T5.3}
In T5.3 we explored how DAOs try to govern themselves. Now we face a deeper question: in a world of digital money, \textbf{who can see what you do with your money}---and should they?
\end{block}

\vspace{5mm}
\begin{columns}[T]
\begin{column}{0.48\textwidth}
\textbf{From Governance to Privacy:}
\begin{itemize}
\item T5.3 asked: Who decides the rules?
\item T5.4 asks: Who watches you follow them?
\item Governance and surveillance are two sides of the same coin
\end{itemize}
\end{column}
\begin{column}{0.48\textwidth}
\textbf{What We Will Cover:}
\begin{itemize}
\item Privacy vs. transparency tradeoffs
\item Privacy technologies and how they work
\item Blockchain surveillance and its tools
\item CBDCs and programmable money
...
\item Post-9/11: After the September 11, 2001 terrorist attacks, governments worldwide dramatically expanded financial surveillance laws, requiring banks to monitor and report suspicious transactions more aggressively than ever before
\end{itemize}
\end{block}
...
\item \textcolor{dfblue}{FATF}: The Financial Action Task Force---an international body that sets anti-money-laundering standards. Its \textbf{Travel Rule} requires financial institutions to share sender and receiver information for transfers above a certain amount---like putting a return address on a package
\end{itemize}
\end{column}
\begin{column}{0.5\textwidth}
...
\textbf{Recall from Day 3}: Bitcoin transactions are \textbf{pseudonymous}---they are linked to addresses (like account numbers) rather than real names. But these addresses can potentially be traced back to real identities.
...
\item Apartheid South Africa: financial records identified anti-apartheid activists
...
% ============================================================================
% SLIDE 8a: Privacy Techniques - Hiding the Sender (C23 split 1/3)
% ============================================================================
\begin{frame}{Privacy Techniques: Hiding the Sender}

\textit{Note: If you covered the optional Topic 4.5 on zero-knowledge proofs, parts of this section will be a review. If not, don't worry---we explain everything you need here.}

\vspace{3mm}
\begin{columns}[T]
\begin{column}{0.48\textwidth}
\textbf{Ring Signatures (used by Monero):}
\begin{itemize}
\item Imagine 10 people each have a key. One of them signs a document, but nobody can tell \textit{which} one signed it
\item The signature is valid, but the signer is hidden among the group
\item \textbf{Analogy:} Like signing a petition where any of 10 members could have written it---the signature is real, but the author is unknown
\end{itemize}
\end{column}
\begin{column}{0.48\textwidth}
\textbf{CoinJoin / Mixing:}
\begin{itemize}
\item \textbf{CoinJoin}: Multiple people combine their transactions into one, making it hard to trace who paid whom. \textbf{Analogy:} Like several people putting cash into a hat and each taking out the same amount---observers cannot tell who paid whom
\item Used by: Dash (``PrivateSend''), Wasabi Wallet, JoinMarket
\item \textbf{Tumbling (mixing) services}: Third parties that mix many people's crypto together to break the trail. Controversial because they are also used for money laundering
\end{itemize}
\end{column}
\end{columns}
\end{frame}
...
% ============================================================================
% SLIDE 8b: Privacy Techniques - Hiding the Receiver (C23 split 2/3)
% ============================================================================
\begin{frame}{Privacy Techniques: Hiding the Receiver}
\begin{columns}[T]
\begin{column}{0.48\textwidth}
\textbf{Stealth Addresses (used by Monero):}
\begin{itemize}
\item Like having a PO box that automatically creates a new, unique address for every letter you receive
\item The sender can send to you, but observers cannot link your different addresses together
\item Each transaction creates a one-time address---even if someone knows your public address, they cannot tell which transactions are yours
\end{itemize}

\vspace{3mm}
\textbf{Result:}\\
The \textit{receiver} of every transaction is hidden from outside observers.
\end{column}
\begin{column}{0.48\textwidth}
\textbf{Mixing Protocols on Ethereum:}
\begin{itemize}
\item \textbf{Tornado Cash}: Users deposit ETH into a smart contract (recall: a program that runs automatically on a blockchain), then withdraw to a different address
\item Zero-knowledge proofs verify the deposit was valid without revealing which deposit matches which withdrawal
\item Breaks the on-chain link between sender and receiver
\end{itemize}
...
% ============================================================================
% SLIDE 8c: Privacy Techniques - Hiding the Amount (C23 split 3/3)
% ============================================================================
\begin{frame}{Privacy Techniques: Hiding the Amount}
\begin{columns}[T]
\begin{column}{0.48\textwidth}
\textbf{RingCT (Ring Confidential Transactions):}
\begin{itemize}
\item Used by Monero---combines ring signatures with hidden amounts
\item Not only is the sender hidden, but the amount transferred is also encrypted
\item The blockchain can still verify that no money was created or destroyed (inputs = outputs), without anyone seeing the actual numbers
\end{itemize}

\vspace{3mm}
\textbf{Bulletproofs:}
\begin{itemize}
\item A type of \textbf{range proof}: a way to prove a number is within a valid range (e.g., ``my balance is between 0 and 1,000'') without revealing the exact number
\item \textbf{Analogy:} Like proving you are old enough to drink without showing your exact birthday
\item Used by Monero to make RingCT more efficient
\end{itemize}
\end{column}
\begin{column}{0.48\textwidth}
\textbf{Zero-Knowledge Proofs for Amounts:}
\begin{itemize}
\item \textbf{Analogy:} Imagine proving you know a secret password without ever saying the password out loud. That is a zero-knowledge proof---you prove you \textit{know} something without \textit{revealing} what you know
\item \textbf{zk-SNARKs} (used by Zcash): Very fast to verify, but require a \textbf{trusted setup}---a one-time ceremony to create the system's initial parameters. If anyone involved cheats during setup, they could create fake proofs forever
\item \textbf{zk-STARKs}: No trusted setup needed, but proofs are larger
\end{itemize}
...
\textbf{More Formally:}
\begin{itemize}
\item Cryptographic method where a prover convinces a verifier of a statement's truth
...
\item You prove they are different colors
...
\begin{itemize}
\item \textbf{zk-SNARKs}: Used by Zcash. Fast verification, but requires a \textbf{trusted setup}---like building a vault where the builders must be trusted, even though later users don't need to trust each other
\item \textbf{zk-STARKs}: No trusted setup needed, but larger proofs
\item \textbf{Bulletproofs}: Monero's \textbf{range proofs}---prove a value is in a valid range without showing the value
\end{itemize}
\end{block}
\end{column}
\end{columns}
...
% ============================================================================
% SLIDE 10a: The Technology of Surveillance (C24 split 1/2)
% ============================================================================
\begin{frame}{Privacy Coin Deep Dive: Monero's Privacy Stack}
\begin{columns}[T]
\begin{column}{0.55\textwidth}
\textbf{Monero (XMR)---Privacy by Default:}
\begin{itemize}
\item \textbf{Ring Signatures:} Mix your transaction with decoys---sender hidden among group. \textbf{Analogy:} Imagine 10 people each have a key; one signs, but nobody can tell which one
\item \textbf{Stealth Addresses:} One-time addresses for each transaction---receiver unlinkable. \textbf{Analogy:} A PO box that auto-generates a new address for every letter received
\item \textbf{RingCT:} Confidential transactions hide amounts while proving no money was created or destroyed
\item \textbf{Dandelion++:} A privacy technique for broadcasting transactions. Instead of announcing a transaction to everyone simultaneously, it first passes through a random chain of nodes---like whispering a message through several people before announcing it publicly
\end{itemize}
\end{column}
\begin{column}{0.42\textwidth}
\textbf{Result:}
...
% ============================================================================
% SLIDE 10b: The Regulation of Privacy (C24 split 2/2)
% ============================================================================
\begin{frame}{The Regulation of Privacy Coins}
\begin{columns}[T]
\begin{column}{0.48\textwidth}
\textbf{How Governments Mandate Transparency:}
\begin{itemize}
\item Exchange delistings: Regulators pressure exchanges to remove privacy coins
\item Travel Rule enforcement: Require sender/receiver data for transfers
\item Outright bans: Some countries prohibit privacy coins entirely
\item Bounties: Governments offer rewards for breaking privacy (e.g., IRS \$625K Monero bounty)
\end{itemize}

\vspace{3mm}
\textbf{The Policy Tension:}
\begin{itemize}
\item Privacy coins have legitimate uses: dissidents, activists, abuse survivors
\item But also used for ransomware and tax evasion
\item Regulators rarely distinguish between the two
\end{itemize}
\end{column}
\begin{column}{0.48\textwidth}
\textbf{Use Cases---Both Sides:}
\begin{itemize}
\item Legitimate: Dissidents and activists under authoritarian regimes
\item Legitimate: Businesses protecting competitive information
\item Illicit: Ransomware payments
\item Illicit: Tax evasion and sanctions circumvention
\end{itemize}

\vspace{3mm}
\begin{block}{Key Question}
Should a technology be banned because it \textit{can} be misused? Cash can also be used for crime, but nobody proposes banning cash entirely.
\end{block}
\end{column}
\end{columns}
\end{frame}
...
\begin{itemize}
\item Ethereum mixing protocol (a smart contract---recall: a program that runs automatically on a blockchain)
...
\begin{itemize}
\item Is code speech? (The First Amendment to the US Constitution protects free speech---relevant because some argue writing code is a form of speech)
...
\vspace{3mm}
\textbf{Pattern-Matching Techniques:}
\begin{itemize}
\item \textbf{Clustering}: Link addresses to same entity
\item Analysts look for clues like ``these addresses always transact together'' (\textbf{common ownership}) or ``this leftover amount went back to the sender'' (\textbf{change detection})
...
\vspace{3mm}
\textbf{Why Pseudonymity Is Weaker:}\\
Pseudonyms can be \textbf{unmasked retroactively}. If anyone ever links your real identity to your Bitcoin address, they can trace ALL your past transactions. \textbf{Analogy:} Like writing under a pen name---once someone discovers your real name, every book you ever wrote is exposed.
\end{column}
\begin{column}{0.5\textwidth}
...
\item Human errors in operational security (OpSec)---the practice of protecting sensitive information from being discovered
\end{itemize}
\end{alertblock}
\end{column}
\end{columns}
...
\begin{columns}[T]
\begin{column}{0.55\textwidth}
\textbf{CBDC (Central Bank Digital Currency) Capabilities:}
\begin{itemize}
\item Complete transaction visibility
\item Programmable spending restrictions
\item Automatic tax collection
\item \textbf{Expiring money (demurrage)}: money that loses value over time if you don't spend it---like a gift card with an expiration date. This encourages spending rather than hoarding
\item Geographic restrictions
\item \textbf{Social credit integration}: Social credit systems assign citizens a score based on their behavior---paying bills on time, obeying traffic laws, or even what they post online. In China's system, a low score can restrict travel, loan access, and even children's school options
...
\item Behavioral prediction: companies use your transaction data to predict what you will buy, how risky you are, and what ads to show you
...
\textbf{The Narrative:}
\begin{itemize}
\item According to the World Bank (2021), 1.4 billion adults remain unbanked globally
...
% ============================================================================
% SLIDE 21b: El Salvador vs M-Pesa Comparison (M5)
% ============================================================================
\begin{frame}{Comparing Inclusion Approaches: What Works?}
\begin{center}
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{|l|l|l|}
\hline
\textbf{Factor} & \textbf{M-Pesa (Kenya)} & \textbf{Bitcoin (El Salvador)} \\
\hline
...
\textbf{What worked}: appropriate technology, human infrastructure, regulatory support.\\
\textbf{What struggled}: cutting-edge technology, top-down mandates, volatile assets.
\end{block}
\end{frame}
...
\textbf{Analogy:} Think of SSI as a digital wallet containing verified credentials---like carrying your passport, diploma, and bank statement in your phone, but you choose which to show and to whom.
...
\item Prove \textbf{creditworthiness} (whether a borrower is likely to repay a loan, based on their financial history) without revealing exact credit score
\item Age verification for services
\item \textbf{Accredited investor} status (a legal designation meaning you meet income or wealth thresholds required to invest in certain high-risk products)
\end{itemize}
...
\textbf{Projects}: Microsoft ION, Civic, Worldcoin (controversial---it collects iris scans, raising concerns about biometric data privacy and consent, especially in developing countries)
\end{column}
\end{columns}
\end{frame}
...
\item \textbf{WikiLeaks} (2010): A website that published secret government documents; banks and payment processors (Visa, Mastercard, PayPal) cut off its funding under government pressure
\item \textbf{Canadian trucker convoy} (2022): Protesters' bank accounts were frozen by government order under emergency powers
\item \textbf{\#EndSARS} (Nigeria, 2020): Protesters against police brutality used crypto to receive donations after banks froze activist accounts
\end{itemize}
\end{column}
\begin{column}{0.5\textwidth}
...
\item Social credit integration (see earlier definition)
\item Tight capital controls
\item Crypto banned domestically
\end{itemize}

\vspace{2mm}
\textbf{India and Africa:}
\begin{itemize}
\item \textbf{India}: 30\% crypto tax imposed (2022); attempted outright ban failed; world's largest biometric ID system (Aadhaar) creates unique privacy tensions
\item \textbf{Africa}: Mobile money revolution (M-Pesa); Nigeria and Kenya lead crypto adoption in Africa, driven by remittances and currency instability
\end{itemize}
...
\item India: Tax and regulate
\item Africa: Mobile-first innovation
\end{itemize}
\end{alertblock}
\end{column}
\end{columns}
...
\item ``Sunlight is the best disinfectant'' (meaning: making information public is the best way to prevent corruption and wrongdoing)
...
% ============================================================================
\begin{frame}{Key Terms \& Definitions (1/3)}
\begin{description}
\item[Financial Privacy] Control over who can see your financial transactions and data.

\item[Pseudonymity] Using identifiers (like Bitcoin addresses) not directly linked to real identity, but potentially traceable. Weaker than anonymity because unmasking one link exposes all history.

\item[Anonymity] True unlinkability between transactions and identity; much stronger than pseudonymity.

\item[Zero-Knowledge Proof] Cryptographic method to prove a statement is true without revealing any additional information. Like proving you know a password without saying it.
...
% ============================================================================
\begin{frame}{Key Terms \& Definitions (2/3)}
\begin{description}
...
\item[Self-Sovereign Identity (SSI)] Identity model where individuals control their own data without centralized authority---like a digital wallet with verified credentials you choose to share.
...
% ============================================================================
% SLIDE 34b: Key Terms (Part 3) -- expanded (M6)
% ============================================================================
\begin{frame}{Key Terms \& Definitions (3/3)}
\begin{description}
\item[Ring Signature] A cryptographic signature that proves one member of a group signed, without revealing which member. Used by Monero to hide the sender.

\item[Stealth Address] A one-time address generated for each transaction, preventing observers from linking transactions to the same receiver.

\item[FATF Travel Rule] Requirement by the Financial Action Task Force that financial institutions share sender/receiver information for transfers above a threshold.

\item[Demurrage] Money that loses value over time if not spent---like a gift card with an expiration date. Encourages spending over hoarding.

\item[Trusted Setup] A one-time ceremony to generate cryptographic parameters. If compromised, fake proofs could be created. Required by zk-SNARKs but not zk-STARKs.

\item[Social Credit System] A system that assigns scores to citizens based on behavior, potentially restricting access to services for low scores.

\item[OpSec] Operational security---the practice of protecting sensitive information from being discovered or exploited.
\end{description}
\end{frame}
