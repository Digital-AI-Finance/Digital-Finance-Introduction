...
\begin{itemize}
\item Bitcoin: 60\%+ annual volatility (Example: If you bought \$100 of Bitcoin, it might be worth \$40-\$160 by year end)
...
\vspace{2mm}
\textit{Quick check: If ``DeFi primitives'' is unfamiliar, briefly review T4.2 first.}
\end{column}
\begin{column}{0.48\textwidth}
...
Stablecoins bridge Traditional Finance (TradFi) and crypto by providing price predictability in a volatile ecosystem.
\end{alertblock}
\end{column}
\end{columns}
\end{frame}
...
USD is the global reserve currency. A dollar-pegged stablecoin inherits this network effect (``network effect'' means more people use dollars, so dollar-based coins are more useful).
\end{block}
\end{column}
\end{columns}
\end{frame}
...
\textbf{Examples:}
\begin{itemize}
\item \textbf{USDT} (Tether): \$110B, largest --- ``Tether USD'' token
\item \textbf{USDC} (Circle): \$35B, regulated --- ``USD Coin'' token
...
\begin{itemize}
\item Arbitrage keeps price at \$1 (arbitrage = buying low in one place and selling high in another until prices match)
...
\vspace{2mm}
\textit{What could go wrong? Example: Tether faced controversy over whether reserves actually existed (2017-2021).}
\end{column}
...
\item If ETH drops, liquidation (like a pawn shop loan --- if your collateral loses value, they sell it to cover the loan)
\end{enumerate}

\vspace{0.3cm}
\textbf{MakerDAO/DAI:}
\begin{itemize}
\item DAI is the largest crypto-backed stablecoin
...
\vspace{3mm}
\begin{alertblock}{Liquidation Cascades: The Domino Effect}
...
\vspace{2mm}
\textit{Think of dominoes: When one person is forced to sell, prices drop, forcing more sales, creating a chain reaction.}
\end{alertblock}
\end{frame}
