...
\begin{enumerate}
    \item \textbf{Explain} how AMMs (automated price-setting systems) replace traditional exchanges
    \item \textbf{Understand} slippage (price moving against you during a trade) and why larger trades cost more
    \item \textbf{Understand} how DeFi lending protocols automatically adjust interest rates
    \item \textbf{Describe} impermanent loss (temporary loss from providing liquidity) and when it matters
    \item \textbf{Analyze} the composability of DeFi protocols (``money legos'')
    \item \textbf{Evaluate} flash loans (instant uncollateralized loans) as both innovation and attack vector
\end{enumerate}
\end{block}

\vspace{0.3cm}
\textbf{Key Skills:} Understanding AMM intuition, LP token mechanics (LP = Liquidity Provider), risk assessment
...
\begin{itemize}
    \item Uses an order book\footnote{Order book = a list of all buy and sell orders, ranked by price} with bids/asks
    \item Market makers\footnote{Market makers = firms that continuously offer to buy and sell, providing liquidity} quote prices
...
\begin{itemize}
    ...
    \item Liquidity pools\footnote{Liquidity pool = a shared fund of two tokens locked in a smart contract, enabling trades} replace market makers
...
\vspace{0.2cm}
\textbf{Leading AMM Protocols:} Uniswap (most popular), Curve (optimized for stablecoins), Balancer (multi-token pools)
\end{frame}
...
% =====================================================================
\begin{frame}{How AMMs Set Prices: The Seesaw Principle}
\begin{columns}[T]
\begin{column}{0.45\textwidth}
\begin{block}{The Core Idea}
The AMM follows a simple rule: \textbf{as you buy more of one token, its price goes up} because less of it remains in the pool.

\vspace{0.2cm}
Think of it like a \textbf{seesaw}---when one side goes up, the other must go down.
\end{block}

\vspace{0.2cm}
\textbf{In plain language:}
\begin{itemize}
    \item The pool always keeps a ``balance'' between its two tokens
    \item Buying one token makes it scarcer (more expensive)
    \item Selling one token makes it more abundant (cheaper)
    \item The price adjusts automatically with every trade
\end{itemize}

\vspace{0.2cm}
{\footnotesize \textcolor{dfgray}{\textit{The math behind this is in the Appendix for curious students.}}}
\end{column}

\begin{column}{0.52\textwidth}
\begin{tikzpicture}[scale=0.55, transform shape]
    % Axes
    \draw[arrow] (0,0) -- (7,0) node[right] {ETH in pool};
    \draw[arrow] (0,0) -- (0,7) node[above] {USDC in pool};
    ...
    % Annotation
    \node[font=\scriptsize, text width=2.5cm, align=center] at (5.5, 5) {More ETH bought $\rightarrow$ price rises};
\end{tikzpicture}
\end{column}
\end{columns}
...
% =====================================================================
\begin{frame}{AMM in Action: A Trade Story}
\begin{block}{Starting Pool}
\begin{itemize}
    \item The pool holds: \textbf{100 ETH} and \textbf{300,000 USDC}
    \item Current price: 1 ETH = 3,000 USDC
\end{itemize}
\end{block}

\textbf{You want to buy 10 ETH. Here's what happens:}
\begin{enumerate}
    \item You send USDC to the pool and ask for 10 ETH
    \item The pool gives you 10 ETH, leaving \textbf{90 ETH} behind
    \item Because ETH is now scarcer in the pool, its price went \textit{up} during your trade
    \item The pool rebalances to about \textbf{333,333 USDC}
    \item You paid \textbf{33,333 USDC} for 10 ETH --- that's \textbf{3,333 USDC per ETH}
\end{enumerate}

\begin{alertblock}{The Cost of a Big Trade}
\begin{itemize}
    \item At the starting price, 10 ETH would cost 30,000 USDC
    \item You actually paid 33,333 USDC --- that's \textbf{3,333 USDC extra}
    \item This ``extra cost'' is \textbf{slippage}: about \textbf{11\%} price impact
    \item \textbf{Lesson}: Bigger trades move the price more against you
\end{itemize}
\end{alertblock}
\end{frame}
...
\begin{enumerate}
    ...
    \item First depositor sets the initial ratio
...
% =====================================================================
% SLIDE 16: IMPERMANENT LOSS - HOW MUCH IT COSTS
% =====================================================================
\begin{frame}{How Much Can Impermanent Loss Cost You?}
\begin{columns}[T]
\begin{column}{0.45\textwidth}
\textbf{The key insight:}\\
If you provide liquidity and one token's price changes significantly, you end up with \textbf{less value} than if you had simply held the tokens. The bigger the price move, the larger the loss.
...
\vspace{0.2cm}
{\footnotesize \textcolor{dfgray}{\textit{The formula behind this table is in the Appendix.}}}
\end{column}

\begin{column}{0.52\textwidth}
\begin{tikzpicture}[scale=0.55, transform shape]
    % Axes
    \draw[arrow] (0,0) -- (7,0) node[right, font=\small] {Price Change};
    ...
    % Annotation
    \node[font=\scriptsize, text width=2.5cm] at (5, 4) {IL grows faster as price diverges more};
\end{tikzpicture}
\end{column}
\end{columns}
...
\textbf{Mitigating IL:}
\begin{itemize}
    \item Provide liquidity to correlated pairs (stablecoin-stablecoin: minimal price divergence)
...
% =====================================================================
\begin{frame}{Impermanent Loss: A Story in Numbers}
\begin{columns}[T]
\begin{column}{0.48\textwidth}
\textbf{You start by providing liquidity:}
\begin{itemize}
    \item You deposit: 1 ETH + 2,000 USDC
    \item Total value: \$4,000
    \item ETH price at deposit: \$2,000
\end{itemize}

\vspace{0.3cm}
\textbf{Then ETH doubles to \$4,000:}
\begin{itemize}
    \item Arbitrage traders rebalance the pool
    \item Your share now has \textit{less} ETH and \textit{more} USDC
    \item Specifically: about 0.707 ETH + 2,828 USDC
    \item Your pool value: \textbf{\$5,657}
\end{itemize}
\end{column}

\begin{column}{0.48\textwidth}
\textbf{What if you had just held instead?}
\begin{itemize}
    \item 1 ETH at \$4,000 = \$4,000
...
\begin{itemize}
    \item Holding: \$6,000
    \item Pool: \$5,657
    \item \textbf{You lost \$343 (about 5.7\%)} by being in the pool
\end{itemize}

\vspace{0.2cm}
\begin{alertblock}{The Break-Even Question}
If the pool earned more than \$343 in trading fees during this time, you still come out ahead. If not, you would have been better off just holding.
\end{alertblock}
\end{column}
\end{columns}
\end{frame}
...
\begin{columns}[T]
\begin{column}{0.45\textwidth}
\textbf{How it works (supply \& demand):}

Lending protocols automatically adjust interest rates:
\begin{itemize}
    \item \textbf{Few borrowers} (low utilization) $\rightarrow$ rates stay \textit{low} to encourage borrowing
    \item \textbf{Many borrowers} (high utilization) $\rightarrow$ rates rise \textit{sharply} to attract more lenders
    \item There is a ``\textbf{kink}'' point (typically 80\%) where rates jump steeply
\end{itemize}

\vspace{0.2cm}
\textbf{Why the kink?}\\
The protocol \textit{wants} some funds always available for withdrawals. The steep rate above 80\% discourages draining the pool completely.

\vspace{0.2cm}
{\footnotesize \textcolor{dfgray}{\textit{The mathematical model is in the Appendix.}}}
\end{column}
...
\begin{tikzpicture}[scale=0.55, transform shape]
    ...
    \node[left, font=\tiny] at (0, 0.5) {Low};
    \node[left, font=\tiny] at (0, 1.5) {Moderate};
    \node[left, font=\tiny] at (0, 5.5) {Very high};

    % Annotations
    \node[font=\scriptsize, text width=2cm, align=center] at (2.5, 3.5) {Gentle slope:\\``Borrow freely''};
    \node[font=\scriptsize, text width=2cm, align=center] at (6, 3.5) {Steep slope:\\``Slow down!''};

    % Kink label
    \node[font=\scriptsize, dfred] at (5, 2.2) {Kink point};
\end{tikzpicture}
\end{column}
\end{columns}
...
\begin{itemize}
    ...
    \item Health Factor: 1.65 (safe zone---above 1.0)
    \item Liquidation at: Health Factor below 1.0
\end{itemize}
...
\begin{itemize}
    ...
    \item Health Factor drops to 0.99
...
\begin{itemize}
    \item Practical understanding of how AMMs price tokens
...
\begin{alertblock}{Optional: For Students with Programming Experience}
The code below shows the inner workings of an AMM. In the notebook, all code runs automatically---you will interact with the AMM through simple inputs and observe the results visually.
\end{alertblock}
...
class SimpleAMM:
    def __init__(self, reserve_x, reserve_y):
        self.x = reserve_x  # ETH reserves
        self.y = reserve_y  # USDC reserves
        self.k = reserve_x * reserve_y  # constant product

    def get_spot_price(self):
        return self.y / self.x  # Price of ETH in USDC
...
\begin{itemize}
    \item \textbf{AMMs} replace order books with liquidity pools and automatic pricing (the ``seesaw'' principle)
...
\begin{column}{0.48\textwidth}
\begin{itemize}
    \item AMM pricing = seesaw principle
    \item Bigger trades = more slippage
\end{itemize}
\end{column}
\begin{column}{0.48\textwidth}
\begin{itemize}
    \item IL grows with price divergence
    \item Interest rates follow supply \& demand
\end{itemize}
\end{column}
\end{columns}

\vspace{0.2cm}
{\footnotesize \textcolor{dfgray}{\textit{Mathematical formulas for all concepts are available in the Appendix slides.}}}
\end{frame}
...
\begin{tikzpicture}[scale=0.65, transform shape]
    ...
    % AMM sub-nodes
    \node[font=\tiny, text width=2cm, align=center] at (-5, 0.8) {Seesaw pricing\\LP Tokens\\Slippage\\IL};
...
The mathematical rule behind AMM pricing (see Appendix).
\end{column}
...
\vspace{0.3cm}
\begin{block}{Question 2: Impermanent Loss}
If you provide liquidity to an ETH/USDC pool and ETH doubles in price, approximately how much impermanent loss do you face?
\begin{enumerate}[A.]
    \item 0\% --- price changes don't affect liquidity providers
    \item About 2\% --- a small cost
    \item About 5.7\% --- a meaningful cost that fees need to offset
    \item About 50\% --- you lose half your money
\end{enumerate}
\end{block}
\end{frame}
...
\begin{itemize}
    ...
    \item Question 2: \textbf{C} --- A 2x price change causes about 5.7\% IL (see table and Appendix)
...
\textbf{Notebook NB09}: AMM Simulation -- practice with AMM pricing, slippage, and impermanent loss
\end{block}
\end{frame}
...
% =====================================================================
% APPENDIX: OPTIONAL MATHEMATICAL DEEP DIVES
% =====================================================================
\appendix

% =====================================================================
% APPENDIX SLIDE A1: AMM MATHEMATICS
% =====================================================================
\begin{frame}{Optional Deep Dive: AMM Mathematics}
\begin{block}{The Constant Product Formula}
\[
x \cdot y = k
\]
\begin{itemize}
    \item $x$ = Token A reserves, $y$ = Token B reserves, $k$ = constant (invariant)
\end{itemize}
\end{block}

\textbf{Price Determination:}
The spot price of token A in terms of token B is the ratio of reserves:
\[
\text{Price of A in B} = \frac{y}{x}
\]

\textbf{Calculating the output of a swap:}
When you add $\Delta x$ of token A, you receive:
\[
\Delta y = y - \frac{k}{x + \Delta x}
\]

\textbf{Example:} Pool has 100 ETH and 300,000 USDC ($k = 30{,}000{,}000$). Swap 10 ETH in:
\begin{itemize}
    \item New ETH = 110, New USDC = $30{,}000{,}000 / 110 = 272{,}727$
    \item You receive $300{,}000 - 272{,}727 = 27{,}273$ USDC
    \item Slippage: $(30{,}000 - 27{,}273) / 30{,}000 = 9.09\%$
\end{itemize}
\end{frame}

% =====================================================================
% APPENDIX SLIDE A2: IMPERMANENT LOSS FORMULA
% =====================================================================
\begin{frame}{Optional Deep Dive: Impermanent Loss Formula}
\begin{block}{IL Formula}
\[
IL = \frac{2\sqrt{r}}{1+r} - 1
\]
where $r = \frac{P_1}{P_0}$ is the price ratio (new price / original price).
\end{block}

\textbf{Derivation intuition:}
\begin{enumerate}
    \item In a constant-product pool, if the price ratio changes to $r$, your holdings rebalance to $\sqrt{r}$ times the original token A and $\frac{1}{\sqrt{r}}$ times token B
    \item The value of your pool position is $\frac{2\sqrt{r}}{1+r}$ of what simple holding would be worth
    \item The difference is impermanent loss
\end{enumerate}

\textbf{Worked example (ETH doubles, $r = 2$):}
\begin{itemize}
    \item $IL = \frac{2\sqrt{2}}{1+2} - 1 = \frac{2 \times 1.414}{3} - 1 = 0.943 - 1 = -5.72\%$
    \item On a \$6,000 holding value: loss = $6{,}000 \times 0.0572 = \$343$
\end{itemize}
\end{frame}

% =====================================================================
% APPENDIX SLIDE A3: ALGORITHMIC INTEREST RATE MODEL
% =====================================================================
\begin{frame}{Optional Deep Dive: Algorithmic Interest Rate Model}
...
R_{\text{borrow}} =
\begin{cases}
R_0 + U \cdot R_{\text{slope1}} & \text{if } U < U_{\text{optimal}} \\[6pt]
R_0 + U_{\text{optimal}} \cdot R_{\text{slope1}} + (U - U_{\text{optimal}}) \cdot R_{\text{slope2}} & \text{if } U \geq U_{\text{optimal}}
\end{cases}
\]

\textbf{Supply Rate} (what lenders earn):
\[
R_{\text{supply}} = R_{\text{borrow}} \times U \times (1 - \text{protocol fee})
\]

\textbf{Typical parameters (Aave):}
\begin{itemize}
    \item $U_{\text{optimal}} = 80\%$, $R_0 = 0\%$, $R_{\text{slope1}} = 4\%$, $R_{\text{slope2}} = 75\%$
    \item At 50\% utilization: rate $\approx 2\%$. At 90\% utilization: rate $\approx 10.7\%$. At 100\%: rate $\approx 18.2\%$
\end{itemize}
\end{frame}
