...
\begin{enumerate}
    ...
    \item \textbf{Understand} the Ethereum Virtual Machine (EVM) and gas mechanism
    \item \textbf{Read} basic Solidity smart contract code
    \item \textbf{Identify} key limitations: oracle problem, immutability, reentrancy risks
...
\begin{itemize}
    \item \textbf{Blockchain}: Distributed, immutable ledger
    \item \textbf{Consensus}: Agreement without central authority
    \item \textbf{Cryptography}: Hashes secure data integrity
    \item \textbf{Wallets}: Private/public key pairs for identity
    \item \textbf{Transactions}: Signed messages changing state
\end{itemize}
\end{column}
...
\begin{itemize}
    ...
    \item Ethereum: ``World computer'' (Turing-complete programs)
\end{itemize}

\vspace{0.3cm}
\textbf{Two Types of Accounts:}
\begin{enumerate}
    \item \textbf{Externally Owned Accounts (EOA)}
    \begin{itemize}
...
\textbf{Key Properties:}
\begin{itemize}
    \item \textbf{Stack-based}: 256-bit word size
    \item \textbf{Sandboxed}: Cannot access host system
...
% =====================================================================
\begin{frame}{State Variables vs. Local Variables}
\begin{columns}[T]
\begin{column}{0.48\textwidth}
...
\begin{itemize}
    \item Modifiers run \textbf{before} the function body (at the \code{\_})
