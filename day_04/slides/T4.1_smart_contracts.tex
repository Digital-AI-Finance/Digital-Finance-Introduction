...
\footnotesize
    ...
    \item Transaction waits in mempool\textsuperscript{a}
    \item Validator\textsuperscript{b} selects transaction for block
    \item Ethereum Virtual Machine (EVM) executes bytecode\textsuperscript{c}
...
\vspace{-0.2cm}
{\scriptsize
\textsuperscript{a}\textbf{Mempool}: a waiting room for unconfirmed transactions. \quad
\textsuperscript{b}\textbf{Validator}: a network participant that confirms transactions. \quad
\textsuperscript{c}\textbf{Bytecode}: machine-readable instructions.
}
\end{frame}
...
You don't need to understand the technical details. The key idea: the EVM is a \textbf{virtual computer that every Ethereum node runs identically}, so everyone agrees on the result.
\end{alertblock}
...
\vspace{0.2cm}
\begin{alertblock}{No Coding Required}
You won't need to write Solidity in this course. The following slides show code examples so you can \textit{read} what a smart contract looks like.
\end{alertblock}
...
\begin{itemize}
    ...
    \item Compiles to EVM bytecode (machine-readable instructions)
    \item Built-in support for ETH transfers
\end{itemize}
\end{column}
...
\begin{alertblock}{Optional: Code Deep Dive}
\small The following slides show real smart contract code. This is \textbf{optional material} for those interested in programming. The key concepts are explained in plain language alongside each code example.
\end{alertblock}
...
\vspace{-0.2cm}
\begin{block}{Plain-English Summary}
\scriptsize This is a simple \textbf{escrow} (a middleman that holds money). A buyer deposits funds; only the buyer can release them to the seller. The contract holds the money until the buyer says ``release.'' No bank or lawyer needed.
\end{block}
\end{frame}
...
{\small\textit{If the code on the previous slide looked intimidating, don't worry---here is a plain-English guide to reading it.}}
...
\begin{exampleblock}{Optional Advanced Material}
\small State variables are \textbf{permanent data stored on the blockchain} (like entries in a shared database). Local variables exist \textbf{only during one function call} and are then discarded (like scratch paper).
\end{exampleblock}
...
\begin{exampleblock}{Optional Advanced Material}
\small Smart contracts can have \textbf{public functions} (anyone can call them) and \textbf{private functions} (only the contract itself can call them)---like the difference between a shop's front door and its staff-only entrance.
\end{exampleblock}
...
\begin{exampleblock}{Optional Advanced Material}
\small Modifiers are \textbf{reusable checks} that run before a function executes---like a bouncer checking IDs before letting someone into a club. If the check fails, the function never runs.
\end{exampleblock}
...
\begin{exampleblock}{Optional Advanced Material}
\small Events are \textbf{notifications} that a smart contract sends out when something happens---like a receipt you get after a purchase. External applications can ``listen'' for these events.
\end{exampleblock}
...
\begin{enumerate}
    \item \textbf{Compile}: Solidity $\rightarrow$ EVM bytecode + ABI\textsuperscript{a}
    ...
    \item \textbf{Generate Address}: From deployer address + nonce\textsuperscript{b} (deterministic)
    \item \textbf{Store Code}: Runtime bytecode stored at new address---\textbf{immutable}
\end{enumerate}

\vspace{0.1cm}
{\scriptsize
\textsuperscript{a}\textbf{ABI} = Application Binary Interface---a menu listing what the contract can do (its public functions and their inputs/outputs).\\
\textsuperscript{b}\textbf{Nonce} = a counter ensuring each transaction is unique, preventing replay attacks.
}
\end{frame}
...
{\small\textit{These risks explain why smart contract security is so important. You don't need to understand the technical details of each attack---focus on the big picture.}}
...
\begin{alertblock}{Optional: Technical Deep Dive}
\small The reentrancy attack exploits a \textbf{timing flaw}: a malicious contract calls back into the victim before the first transaction finishes, draining funds repeatedly. The fix: \textbf{always update your records BEFORE sending money.}
\end{alertblock}
...
\small In the notebook, all code runs automatically. You only need to \textbf{observe the outputs and answer the analysis questions}---no programming required.
\end{alertblock}
