...
\vspace{0.3cm}
\begin{alertblock}{The Howey Test\footnotemark{} (Plain English)}
US courts use 4 criteria to determine if something is a security: (1) investment of money, (2) in a common enterprise, (3) with expectation of profits, (4) from others' efforts. If all 4 apply, it's a security requiring SEC\footnotemark{} registration.
\end{alertblock}
\footnotetext[1]{Howey Test = the US legal test (from a 1946 Supreme Court case) for whether something is a security.}
\footnotetext[2]{SEC = Securities and Exchange Commission, the main US financial regulator overseeing securities markets.}
\end{frame}
...
{\scriptsize *Zero-knowledge proofs = a way of proving something is true without revealing the underlying data (e.g., proving you have enough funds without showing your balance).\\
**Disintermediation = removing banks as middlemen between central banks and customers.}
\end{frame}
...
{\scriptsize *NFC = Near-Field Communication (tap-to-pay technology).\\
**Store-and-forward = save the transaction locally, send when internet returns.\\
***Eventual consistency = the central ledger catches up when devices reconnect.}
\end{column}
