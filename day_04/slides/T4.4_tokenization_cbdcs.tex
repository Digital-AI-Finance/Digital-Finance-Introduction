...
\begin{itemize}
    \item Blockchain fundamentals (T1.3)
    \item Smart contracts basics (T4.1)
    \item Stablecoin mechanisms (T4.3)
...
\begin{itemize}
    ...
    \item Token standards (ERC-20, ERC-721)
    \item Fiat-backed vs. algorithmic stability
\end{itemize}
\end{column}

\begin{column}{0.48\textwidth}
\begin{block}{Core Understanding}
Tokens are programmable digital representations that can:
\begin{itemize}
    \item Transfer value 24/7
    \item Execute automated logic
    \item Enable fractional ownership
    \item Operate across borders
\end{itemize}
\end{block}

\vspace{0.2cm}
\textbf{New Ground:} We now extend tokenization to traditional assets and sovereign money.
\end{column}
\end{columns}
\end{frame}
...
Boston Consulting Group projects \textbf{\$16 trillion} in tokenized assets by 2030. Over \textbf{130 countries} are exploring CBDCs.
\end{block}
\end{frame}
...
\vspace{0.3cm}
\begin{alertblock}{Regulatory Gray Zone}
The Howey Test determines if a token is a security. Many tokens blur the line, leading to SEC enforcement actions. When in doubt, assume it's a security.
\end{alertblock}
\end{frame}
